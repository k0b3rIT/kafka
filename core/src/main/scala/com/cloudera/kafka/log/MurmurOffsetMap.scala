...
package com.cloudera.kafka.log

import com.cloudera.kafka.common.utils.Murmur3
 ...
 */
@nonthreadsafe
class MurmurOffsetMap(val memory: Int) extends OffsetMap {
  ...
  private def positionOf(hash: ByteBuffer, attempt: Int): Int = {
    val probe = ByteUtils.readIntBE(hash.array(), math.min(attempt, hashSize - 4)) + math.max(0, attempt - hashSize + 4)
