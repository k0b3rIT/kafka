...
import org.apache.kafka.server.config.{DelegationTokenManagerConfigs, ZkConfigs}
...
object Kafka extends Logging {
  ...
  def generateDelegationTokenPassword(props: Properties): Properties = {
    ...
    val masterKeyKey: String = DelegationTokenManagerConfigs.DELEGATION_TOKEN_SECRET_KEY_ALIAS_CONFIG
    val secretKeyKey: String = DelegationTokenManagerConfigs.DELEGATION_TOKEN_SECRET_KEY_CONFIG
    val enabledKey: String = "delegation.token.enable"

    if ("true" == props.getProperty(enabledKey)) {
      debug(s"Try to generate password for $secretKeyKey.")
      generatePasswordForKey(secretKeyKey, props, generatedProps)

      if (!generatedProps.containsKey(secretKeyKey)) {
        debug(s"The $secretKeyKey not found. Try to generate password for $masterKeyKey.")
        generatePasswordForKey(masterKeyKey, props, generatedProps)
      }
    }

    props.remove(generatorKeyFor(masterKeyKey))
    props.remove(generatorKeyFor(secretKeyKey))
